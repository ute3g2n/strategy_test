...
const screen08: ScreenDefinition = {
  ...
  title: 'Backtest条件設定',
  navId: 'NAV-02',
  description: 'BacktestProductの条件設定',
  defaultState: 'REQUIRED',
  e2eId: 'BacktestProduct-UI-01',
}
...
function installApiMock(runs: unknown[] = []) {
  const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (url.endsWith('/api/backtest-product/catalog')) return response(catalog)
    if (url.endsWith('/api/backtest/runs')) return response({ items: runs })
    if (url.endsWith('/api/backtest-product/backtest/preflight')) return response(insufficient, 422)
    if (url.endsWith('/api/backtest-product/timeframe-generation-jobs')) {
...
describe('BacktestProduct Web Product UI', () => {
  it('uses only the five strategy timeframes and sends the missing-data journey to the local generation API', async () => {
    ...
    render(<BacktestProductScreen screen={screen08} onOpenLegacy={() => undefined} />)

    await waitFor(() => expect(screen.getByTestId('backtest-product-catalog-table')).toBeInTheDocument())
    ...
    expect(screen.getByTestId('backtest-product-generation-form')).toBeInTheDocument()
    ...
    const generationCall = fetchMock.mock.calls.find(([url]) => String(url).endsWith('/api/backtest-product/timeframe-generation-jobs'))
  ...
  it('serializes BacktestProduct condition datetime pickers as UTC and blocks a reversed range', async () => {
    const user = userEvent.setup()
    const fetchMock = installApiMock()
    render(<BacktestProductScreen screen={screen08} onOpenLegacy={() => undefined} />)
    ...
    await user.click(screen.getByRole('button', { name: '事前確認' }))
    const preflightCalls = () => fetchMock.mock.calls.filter(([url]) => String(url).endsWith('/api/backtest-product/backtest/preflight'))
  ...
  it('does not call the preflight API when a condition endpoint is empty', async () => {
    ...
    expect(fetchMock.mock.calls.some(([url]) => String(url).endsWith('/api/backtest-product/backtest/preflight'))).toBe(false)
  ...
  it('does not create a generation job when the selected range exceeds source coverage', async () => {
    ...
    expect(fetchMock.mock.calls.some(([url]) => String(url).endsWith('/api/backtest-product/timeframe-generation-jobs'))).toBe(false)
  ...
  it('shows the approved bounded DELETE-G1 boundary and has no automated axe violations in the actual-result screen shell', async () => {
    ...
    const { container } = render(<BacktestProductScreen screen={resultScreen} onOpenLegacy={() => undefined} />)

    await waitFor(() => expect(screen.getByTestId('backtest-product-delete-gate')).toBeInTheDocument())
  ...
  it('requires confirmation before requesting deletion for a completed Run', async () => {
    ...
    const resultScreen: ScreenDefinition = { ...screen08, id: 'SCREEN-10', title: 'Backtest結果サマリー' }
    render(<BacktestProductScreen screen={resultScreen} onOpenLegacy={() => undefined} />)

    await waitFor(() => expect(screen.getByTestId('backtest-product-run-RUN-UI-001')).toBeInTheDocument())
    ...
    expect(fetchMock.mock.calls.some(([input]) => String(input).endsWith('/api/backtest-product/result-artifacts/delete'))).toBe(false)

    await user.click(screen.getByRole('button', { name: '結果表示を削除する' }))
    await waitFor(() => expect(fetchMock.mock.calls.some(([input]) => String(input).endsWith('/api/backtest-product/result-artifacts/delete'))).toBe(true))
    const deleteCall = fetchMock.mock.calls.find(([input]) => String(input).endsWith('/api/backtest-product/result-artifacts/delete'))
