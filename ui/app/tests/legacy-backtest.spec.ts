...
test('P5R-09: real Backtest UI journey captures every manual procedure', async ({ page }, testInfo) => {
  ...
  await expect(page.getByText('Backtest実行範囲')).toBeVisible()
  await capture(page, testInfo, 'BT-MAN-01', 'Backtest範囲とBacktest条件画面を開く', captures)

  await page.getByRole('button', { name: 'Preflight実行' }).click()
  await expect(page.getByTestId('legacy-backtest-preflight-result')).toContainText('TYPE_AND_UNIT')
  ...
  const startInput = page.getByLabel('開始日時（UTC）')
  const endInput = page.getByLabel('終了日時（UTC）')
  await expect(startInput).toHaveAttribute('type', 'datetime-local')
  await expect(startInput).toHaveAttribute('step', '60')
  await startInput.focus()
  await expect(startInput).toBeFocused()
  await startInput.fill('2025-02-24T03:00')
  await page.getByRole('button', { name: 'Preflight実行' }).click()
  await expect(page.getByText('開始日時は終了日時より前にしてください。')).toBeVisible()
  await capture(page, testInfo, 'BT-MAN-03', '開始が終了以降の期間を停止する', captures)

  await startInput.fill('2025-02-24T00:00')
  await page.getByRole('button', { name: 'Preflight実行' }).click()
  await expect(page.getByText('Preflight PASS。実Runを開始できます。')).toBeVisible()
  await page.getByRole('button', { name: 'Single Run開始' }).click()
  await expect(page.getByTestId('legacy-backtest-run-status')).toBeVisible()
  await capture(page, testInfo, 'BT-MAN-04', 'Single Runを開始し、実Run IDと進捗を確認する', captures)
  await expect(page.getByTestId('legacy-backtest-run-status')).toContainText('SUCCEEDED', { timeout: 120_000 })
  await expect(page.getByTestId('legacy-backtest-five-metrics')).toBeVisible()
  await expect(page.getByTestId('legacy-backtest-five-metrics')).toContainText('総損益')
  ...
  await endInput.fill('2025-02-24T04:00')
  await page.getByRole('button', { name: 'Preflight実行' }).click()
  await expect(page.getByText('Preflight PASS。実Runを開始できます。')).toBeVisible()
  await page.getByRole('button', { name: 'Single Run開始' }).click()
  ...
  await expect(page.getByTestId('legacy-backtest-run-status')).toContainText('CANCELLED', { timeout: 10_000 })
  ...
  await endInput.fill('2025-02-24T02:30')
  await page.getByRole('button', { name: 'Preflight実行' }).click()
  await expect(page.getByText('Preflight PASS。実Runを開始できます。')).toBeVisible()
  ...
  await page.getByTestId('legacy-backtest-tab-single').click()
  ...
  await page.getByTestId('legacy-backtest-tab-evaluation').click()
  ...
  expect(blocking, 'legacy Backtest UI critical/serious axe violations').toEqual([])
  expect(externalRequests).toEqual([])
  expect(captures.map((capture) => capture.manual_id)).toEqual(captureIds)

  await mkdir(evidenceRoot, { recursive: true })
  await writeFile(join(evidenceRoot, `capture-registry-${testInfo.project.name}.json`), JSON.stringify({ run_id: 'RUN-P5R-09-20260816-001', project: testInfo.project.name, test: 'legacy-backtest.spec.ts', external_requests: externalRequests, axe_blocking_violations: blocking, captures }, null, 2), 'utf-8')
})
