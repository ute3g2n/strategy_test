...
| S4.4-TS-003 | High | OPEN | `human_gate_policy=P2-IC-HG-01` に対する権限者署名済み承認が存在しない。作業 Agent 自己承認は禁止されている。 | `tests/evidence/phase2/RUN-P2-IC-001` に署名検証可能な承認 JSON がないことを確認 | 必須。worktree 外で権限者が署名し、Runner の公開鍵検証を通す | 未実施。Human Gate へ進めない |
