...
description: 新規Markdown・HTML文書または大幅変更文書を1ファイル単位で判定し、ローカルcontext manifestの追加・更新・blocked判定をstrict JSONで返す。
---
...
文書を保存したときに、案内図（context manifest）へ載せるべき情報を安全に更新する。日常の軽微な変更では意味的レコードを不要に再生成できるが、hashは必ず決定的に更新する。コードの構文解析、Git操作、commit前Gateは本Skillの責務外とし、決定的なローカル処理へ渡す。
...
- `source_hash`: SHA-256の小文字64桁。取得元と計算時点をreceiptに残す。
...
- `request_id`、`schema_version`、`generator_version`、`input_hash`、`receipt`。

## 出力契約

追加の説明文やMarkdownを混ぜず、次のキーだけを持つstrict JSONを返す。`additionalProperties`は禁止する。

`artifact_id`、`action`、`summary`、`purpose`、`triggers`、`headings`、`relations`、`confidence`、`reason`、`source_hash`、`receipt`

`action` は次のいずれかとする。

- `record_add`: 新規文書。既存レコードがあってはならない。
- `record_update`: 大幅変更により要約、目的、見出し、関係の更新が必要。
- `metadata_unchanged`: 大幅変更ではなく、意味的レコードは維持してhash・更新時刻・receiptだけを更新。
- `blocked`: 入力不正、対象外、Secret疑い、path境界違反、hash不一致、excerpt上限超過、判断材料不足、schema不整合。

## 実行手順

1. `relative_path`を正規化し、repo内の1ファイルであることを確認する。
2. `kind`、拡張子、対象policy、source_hash形式、excerpt長、入力schemaを検証する。
...
6. receiptへ入力hash、source_hash、generator、判定時刻、境界判定、理由を記録する。
...
- source_hash、path、artifact_id、receiptが一致する。
