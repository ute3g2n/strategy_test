...
- 実行基盤準備: `plan/Phase0_Step0前_実行基盤準備.md`
- 機械実行用Prompt列: `plan/Phase0_機械実行用プロンプト列.md`
...
月額1万円は市場データだけの予算ではなく、クラウドVM、監視、バックアップ、Secret管理等を含む総額である。Step 6では固定運用費を先に差し引き、残りを`market_data_budget_jpy`として別管理する。履歴データの一時購入費は月額費と分離し、購入前にHuman Gate H2で承認する。
...
資金額が「100万円未満」とだけ決まっているため、50万円、75万円、100万円未満上限の3Scenarioで計算する。
...
| 点 | 意味 |
|---:|---|
| 0 | Hard Gate相当の重大な不適合、または採点不能。通常は採点対象から外す |
| 1 | 非常に弱い。条件付きでも初期Live候補として扱いにくい |
| 2 | 基準未満。明確な代替手段またはRisk受容が必要 |
| 3 | 許容可能。要件を満たすが特別な優位性はない |
| 4 | 強い。費用、品質、運用面で明確な利点がある |
| 5 | 非常に強い。公式Evidenceと複数期間の数値で優位性が確認できる |

小項目ごとの数値Thresholdは、Vehicle classの単位差を考慮してStep 0で事前登録する。Backtest結果確認後にThresholdを変更しない。
...
- Missingを勝手に補間していない。
- Score再計算が決定的。
- 2名以上の独立採点者が、互いの点数と総合順位を見ずに採点している。
- 0～5点尺度で2点以上差がある小項目は、第三のReviewerがEvidenceを確認している。
...
- 他の採点者の点数、総合順位、Candidate人気を参照しない。
- 各点数に計測値、Evidence ID、判定理由、不確実性を付ける。
...
鮮度期限を超えたEvidenceは削除せず`stale`とし、再検証完了までCritical判定の根拠に使用しない。
...
計画承認後の実行は、`plan/Phase0_機械実行用プロンプト列.md`のP00からP13を順番に実行する。具体的なCandidateのWeb検索、Data購入、Backtestは、Prompt列内の該当Human Gateを通過するまで行わない。

### 21.1 実行上の補足

- この文書のStep 0～12のPromptは工程ごとの責務定義である。実際に上から順番に投入するPrompt列は、`plan/Phase0_機械実行用プロンプト列.md`を正本とする。
- 実務上は、最初に`実行基盤の準備`を行い、その後にStep 0から順番に進める。
- ここでいう実行基盤は、専用のAIサブエージェントや新しいSkillを必ず事前実装することを意味しない。
- 最低限必要なのは、役割分担、入出力ファイル、Evidence schema、承認Gate、再実行ルールの確定である。
- 現在のPhase 0では、Role名は`論理役割`として定義している。1人のAIが役割を切り替えて実行してもよいが、採点者と監査者の独立性は維持する。
- 専用オーケストレータ、専用Skill、専用サブエージェントの実装は、Phase 0の調査を反復実行する段階で投資対効果が出る場合に行う。

### 21.2 Step 0前の実行基盤準備チェックリスト

Step 0に入る前に、少なくとも次を準備する。

1. 成果物保存先ディレクトリの確定。
2. Candidate schema、Evidence schema、Score schemaの確定。
3. `Unknown`、`Pending`、`Conflict`の扱いルールの確定。
4. Human Gate H0～H5の承認者と承認条件の確定。
5. Web調査時の出典記録フォーマットの確定。
6. Prompt実行ログと再実行条件の確定。

この準備が終われば、専用のサブエージェント実装が未完成でもStep 0へ進める。
