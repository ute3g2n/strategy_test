...
ただし、<code>P5R-H0</code>は未承認である。2026-08-16に<a href="./Phase5R_実行計画書_v0.1_2026-08-16.md">P5R正式実行計画書（P5R-PLAN-001）</a>を作成し、実Application APIに接続したUIをPlaywrightで操作・assert・撮影してHTMLバックテスト手順書へ掲載する工程、及びP5R専用品質Gate scope登録を追加した。これはP5R-H0の承認、P5R実装、外部接続の開始ではない。次に必要なのはP5R-H0の明示承認である。
...
P5Rは要件定義書v3で採用済みであり、正式実行計画書も作成済みである。P5R-H0を開始するには、次の一文による別の承認が必要である。

> Phase 5Rの対象範囲、既存ローカルP5 Dataの利用範囲、保存、固定PCの受入負荷、Walk-forward窓、UI実接続、P5R-AC-01から16、及びP6以降へ残す範囲を承認します。P5R-H0を開始してください。

その承認後に初めて、P5Rの詳細設計、AI部品の再利用判断、P5R専用品質Gate scope設計へ進む。P5R-H1の承認前には実装・実画面撮影を始めない。要件定義書v3、統合台帳、P6入力はすでにロードマップ反映済みだが、P5R実装は始めない。
