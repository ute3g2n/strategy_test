...
| 項目 | 内容 |
|---|---|
| Document ID | `AT-REQ-002` |
| Document set ID | `AT-REQ-V2-SET-001` |
| 版 | `v2.0` |
| 状態 | `FORMAL_V2_BASELINE_APPROVED`（RQV2-H2／RQV2-H3承認済み、RQV2-18完了） |
| 基準日 | 2026-08-11 |
| 生成元candidate | [`01_自動トレードシステム要件定義書_v2_candidate.md`](./drafts/v2/01_自動トレードシステム要件定義書_v2_candidate.md) |
| 生成元candidate SHA-256 | `1F0DF31104002FEC060E183DA2DE46CB8F8A91658FFBE2D44FFF66503B2F34B7` |
| 承認Gate | `RQV2-H2: APPROVED`（2026-08-11、運用者承認）、`RQV2-H3: APPROVED`（2026-08-11、運用者承認） |
| 旧文書との関係 | `AT-REQ-001`／v0.4の後継v2。旧Markdownと旧HTMLは履歴として保持し、上書きしない。 |
| 正式HTML | [`doc/requirements/01_自動トレードシステム要件定義書_v2.html`](../../doc/requirements/01_自動トレードシステム要件定義書_v2.html) |
| 索引 | [`doc/index.html`](../../doc/index.html) |
| UIモック | [`doc/ui_mock/01_自動トレードシステム_UIモック.html`](../../doc/ui_mock/01_自動トレードシステム_UIモック.html) |
...
| 正本方針 | 本v2ではREQ-V2と章00〜62を正本とし、旧文書・旧ID・断片は参照または履歴として扱う。正式HTML、`doc/index.html`、UIモックからの相互リンクをRQV2-16で同期し、RQV2-18で基準線・引渡し状態を確定した |

> 本ファイルはRQV2-H2で承認された最終candidateをRQV2-H3承認後の正式v2基準線へ昇格したものであり、実装済み、実市場性能、利益性、Broker／Paper／Liveの利用可能性を意味しない。未実装・未実証・UNKNOWN・LATER_GATE・BLOCKED・Human Gate待ちは状態を保持する。正式公開後も外部接続・実注文・実資金利用は別Human Gateなしに行わない。
...
6. Phase 4〜11ロードマップは章62.5と独立artifactの両方から参照できるが、各Phase別Human Gateの前に実装・外部接続を発火しない。
