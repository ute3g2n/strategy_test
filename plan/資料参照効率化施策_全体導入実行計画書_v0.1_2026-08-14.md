...
|種別|完全名|固定モデル / reasoning_effort|責務|軽量化境界|
|---|---|---|---|---|
|Skill|`autotrade_skill_context_manifest_maintenance_v0_1`|該当なし|新規／大幅変更文書のマニフェスト判定・更新規約|対象1ファイル、既存レコード、構造化差分だけを入力にする|
|Agent|`AutoTrade_A07_ContextManifestMaintainer_v0_1`|`gpt-5.6-luna / low`|文書レコードの追加、または大幅変更時の意味的更新要否を返す|本文は上限18,000文字、出力はJSON 1レコード、ネットワーク・Secret・本文全量出力禁止|
|Skill|`autotrade_skill_context_routing_v0_1`|該当なし|依頼をマニフェスト検索語へ正規化し、読むべきIDを選ぶ規約|マニフェスト以外を初期入力にしない|
|Agent|`AutoTrade_A08_ContextRouter_v0_1`|`gpt-5.6-luna / low`|最大3主資料と補助資料、JIT取得範囲、欠落情報を構造化出力する|本文を読まない。選定JSONのみを返す|

CTXMAPの現行実行経路では、A07、A08、およびCTX-11で使うA80を `gpt-5.6-luna`、`reasoning_effort=low` に統一する。超軽量化はモデル名だけに依存せず、入力ファイル数・文字数・出力schema・禁止操作を厳格に固定して達成する。過去のPhase／CTX receiptに記録された旧モデル名は、過去の実行事実として書き換えない。

#### 実行プロファイル追補（2026-08-14）

ユーザー指示「gpt-5.1の部分を、gpt-5.6-luna の effort : low に変更して実施」に基づき、現在のCTXMAP実行経路の固定プロファイルを変更した。Agent JSONの `model` と `reasoning_effort`、runtime dispatchの引数、sanitized receiptの3箇所を同じ値で検証する。A07は文書ごとの意味判定、A08はmanifest-only routing、A80は最終HTML／台帳統合に限定し、入力境界・ネットワーク禁止・Secret禁止・fail-closed・独立レビュー必須は変更しない。モデル変更だけでHigh指摘、H1、または独立性不足を解消したことにはしない。
...
|`CTXMAP-H1`|変更監視でA07を自動起動し、auto-commit前に整合ゲートを必須化すること|`APPROVED (2026-08-15)`|保存時の自動モデル実行・自動コミット経路変更・失敗時停止を伴うため|実装・テスト・初回全量マニフェストがPASSし、ユーザーから `CTXMAP-H1を承認します` を受領済み。外部I/O、Secret、実取引、WSL未実行範囲は承認対象外|
...
CTX-09で受入条件を満たし、初回全量マニフェスト、MCPのローカル安全性、commit前fail closed、監視の停止・復旧手順、実行時のAgent呼出し境界を提示した。ユーザーから `CTXMAP-H1を承認します` を受領したため、CTX-10のwatcher有効化リハーサルと詳細解説HTML作成を開始する。
...
#### 2026-08-15 現在状態（現在の正本）

CTXMAP-H1は承認済みで、CTX-11 final precommit GateはPASS（SHA-256 `d788bf5fda2ef77077248d1bbe3f7da0b04f4e8ff3e69af72ce143c765793625`）である。現行正本値はmanifest active=428、code artifacts=255（COMPLETE=217、PARTIAL=38）、relation graph nodes=3319／edges=7021、A07 responses=428、context_index tests=94 passed。A130はPASSしたが、A90がHigh 2件（旧数値、CTX-11 Gate／A130証拠リンク不足）を検出したため、CTX-11は修正待ちでGitへ進めない。A150は未実行であり、文書3点同期→A07再登録→Gate PASS→A90再レビュー→A150の順に進める。WSL依存検証、外部I/O、Secret、実取引、独立child dispatch、PARTIAL/code diagnosticsは未実行境界として残す。詳細は `plan/context_index/CTX-11_最終受入・引渡し.md` を正本とする。

- 目的: 全成果物・証跡・正本導線・運用状態を確定し、安全に引き渡す。
- 主成果物: 最終受入記録、更新済み統合台帳、実行ログ、Git commit/push。
- 受入条件: CTX-00〜10の出力、manifest、MCP、Gate、詳細解説HTML、README、AI基盤HTML、doc/index、台帳が一致し、A90のCritical/Highが0、A150が完了し、今回の変更だけをコミットする。Highが残る間はGitへ進めない。

#### 実行用プロンプト

```text
あなたは CTX-11 の最終統合・引渡し担当です。CTXMAP-H1承認、CTX-10の有効化検証、CTX-11 final Gate、A130検証を確認してから実施してください。A90のHigh指摘が残る間は、受入完了やGitへ進めず、修正待ちとして記録してください。
...
【必須順序】
- 文書3点（CTX-11最終Markdown、実行計画書、統合台帳）を現行証跡へ同期する。
- A07を再登録し、CTX-11 final Gateを再実行してPASSを確認する。
- A90を再レビューし、Critical/High=0を確認する。
- A150を最後に実行する。Orchestrator child dispatch unavailableの場合は、agent_id=N/A、independent=false、SELF_REVIEW_FALLBACKを正直に保持する。
- Highが残る場合、Git stage／commit／pushを行わない。
