...
> **2026-08-14 scope amendment:** このP5-08判定は旧Databento request／Runnerに対する開始前停止の履歴である。ProviderはBinance Data Vision、暫定対象は`BTCUSDT`／`ETHUSDT`へ変更されたため、旧request／Runnerを現行実行へ流用しない。Binance用amendment、固定request、Runner、checksum／hash Evidenceが成立するまで、P5-08は`BLOCKED_SCOPE_CHANGE`のまま維持する。詳細は [Binance方針転換ログ](P5-08_Binance_Data_Vision方針転換・Crypto暫定対象_2026-08-14.md) を参照する。

- Step ID: `P5-08`
- Phase ID: `PHASE5_MARKET_DATA_OPERATIONALIZATION_EVIDENCE_2026_08_12`
- Plan: `P5-PLAN-001`
- Run ID: `RUN-P5-08-BLOCKED-GATE-001`
- 状態: `P5-08_BLOCKED_SCOPE_CHANGE_AND_PREEXECUTION_EVIDENCE_MISSING`
- 判定: `BLOCKED`
...
| ID | 重大度 | 状態 | 内容・処置 |
|---|---|---|---|
| P5-08-F-001 | Medium | STOP / OPEN | 旧Databento範囲のP5-DATA-G1は現行Binance範囲へ読み替えられない。Binance用Provider、Spot、BTCUSDT／ETHUSDT、期間、checksum、利用条件、固定requestのamendmentが未成立であり、P5-08を開始しない。 |
...
| P5-08-F-004 | Medium | CLOSED | 旧Runでは外部I/O、Provider、Secret、費用、実Data取得、Raw／Normalized保存は0件。Provider変更後も、Binance用実行前Evidenceが揃うまでData取得を開始しない。 |
...
| 入力 | 判定 | 不足時の停止 |
|---|---|---|
| 旧Databento P5-DATA-G1承認記録 | 保存済み／履歴 | `HISTORICAL_APPROVED_NOT_CURRENT_BINANCE_SCOPE` |
...
- [旧Databento P5-08 Evidence root／Runner README](../../../tests/evidence/phase5/RUN-P5-08-DATABENTO-001/README.md)
- [旧Databento P5-08 request](../../../tests/evidence/phase5/RUN-P5-08-DATABENTO-001/request.json)
- [旧Databento P5-08 dry-run report](../../../tests/evidence/phase5/RUN-P5-08-DATABENTO-001/logs/dry-run-report.json)
...
上記P5-08 dispatch receiptと旧Databento Evidence rootは2026-08-12時点の開始拒否・旧範囲の履歴である。2026-08-14のBinance方針変更後の現行Provider範囲を承認するEvidenceではない。

旧Databento P5-DATA-G1の範囲は明示承認済みだったが、現行Binance範囲には新しいP5-DATA-G1 amendmentが必要である。Binanceの承認範囲、固定Runner、command、target scope、Secret非使用、利用条件、checksum／hash、外部host isolation、Evidence rootが揃うまで、P5-08は再開しない。事前費用見積りは開始条件にしないが、実行後usage監査は必須とする。
